(function() {
	...
	var localFilesToCache = [
		'.',
		'pwa-stylesheets/css/style.css?${ date?string( "ddMMyyyyHHmm" ) }',
		'pwa-images/404.svg',
	...
	var externalFilesToCache = [
		...
		'https://www.ptlp.co/resource-all/font/font-${ lang }.css'
	];

	var STATIC_VERSION = "${ date?string( 'ddMMyyyyHHmm' ) }";
	...
	var hostName = <#if website == 'ALPHA'>"http://${ website.getHostName() }:8080"<#else>"https://${ website.getHostName() }"</#if>;
	var apiPrefix = <#if website == 'ALPHA'>"https://www-delta.pratilipi.com"<#else>"https://${ website.getHostName() }"</#if>;

	/* Cache Keys */
	var PWA_INDEX_HTML = "app-shell-${ date?string( 'ddMMyyyyHHmm' ) }.html";
...
//				}
	...
	self.addEventListener( 'fetch', function(event) {
		...
		if( url.indexOf( hostName ) > -1
			...
			&& url.indexOf( hostName + "/pwa-sw-${ website.name() }.js" ) === -1
			...
			&& url.indexOf( hostName + "/elements.${ website.getDisplayLanguage().getCode() }" ) === -1
			...
			&& url.indexOf( hostName + "/pwa-manifest-${ website.name() }.json" ) === -1
